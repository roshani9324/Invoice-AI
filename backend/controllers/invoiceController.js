...
// for helper functions for uploading files to public folder
...
//generate unique invoice number to avoid collisions in the DB for invoice number
...
export async function createInvoice(req, res) {
  try {
    ...
    // invoice number if present then error else ok because it must be unique.
...
export async function getInvoiceById(req, res) {
  try {
    ...
    console.error("GETINVOICE BY ID:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
}
...
export async function updateInvoice(req, res) {
  try {
    ...
    //if user changes the invoice number ensure that it not exists already or not in use
